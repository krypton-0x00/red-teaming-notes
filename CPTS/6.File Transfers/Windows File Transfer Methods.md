...
```rb
shakir0x00@htb[/htb]$ sudo impacket-smbserver share -smb2support /tmp/smbshare
...
```powershell
shakir0x00@htb[/htb]$ sudo impacket-smbserver share -smb2support /tmp/smbshare -user test -password test
...
```sh
shakir0x00@htb[/htb]$ sudo pip3 install pyftpdlib
```
...
```shell-session
shakir0x00@htb[/htb]$ sudo python3 -m pyftpdlib --port 21
...
```shell
shakir0x00@htb[/htb]$ echo  | base64 -d > hosts
```

Windows File Transfer Methods

```shell
shakir0x00@htb[/htb]$ md5sum hosts 
...
```shell 
shakir0x00@htb[/htb]$ pip3 install uploadserver
...
```shell
shakir0x00@htb[/htb]$ nc -lvnp 8000
...
```shell-session
shakir0x00@htb[/htb]$ echo <base64> | base64 -d -w 0 > hosts
```
...
```shell-session
shakir0x00@htb[/htb]$ sudo pip3 install wsgidav cheroot
...
```shell-session
shakir0x00@htb[/htb]$ sudo wsgidav --host=0.0.0.0 --port=80 --root=/tmp --auth=anonymous 
...
```shell-session
shakir0x00@htb[/htb]$ sudo python3 -m pyftpdlib --port 21 --write
