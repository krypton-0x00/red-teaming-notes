...
```shell-session
shakir0x00@htb[/htb]$ sudo apt install mysql-server -y
shakir0x00@htb[/htb]$ cat /etc/mysql/mysql.conf.d/mysqld.cnf | grep -v "#" | sed -r '/^\s*$/d'
...
****
```shell-session
shakir0x00@htb[/htb]$ sudo nmap 10.129.14.128 -sV -sC -p3306 --script mysql*
...
```shell-session
shakir0x00@htb[/htb]$ mysql -u root -h 10.129.14.132
...
```shell-session
shakir0x00@htb[/htb]$ mysql -u root -pP4SSw0rd -h 10.129.14.128
