...
```shell-session
shakir0x00@htb[/htb]$ sudo nmap 10.129.14.128 -sV -p110,143,993,995 -sC
...
```shell-session
shakir0x00@htb[/htb]$ curl -k 'imaps://10.129.14.128' --user user:p4ssw0rd
...
```shell-session
shakir0x00@htb[/htb]$ openssl s_client -connect 10.129.14.128:pop3s
...
```shell-session
shakir0x00@htb[/htb]$ openssl s_client -connect 10.129.14.128:imaps
...
### 🔑 First: What are these ports and protocols?

| Port | Protocol | Secure? | Purpose                            |
|------|----------|---------|------------------------------------|
| 110  | POP3     | ❌      | Retrieve emails (basic, unencrypted) |
| 995  | POP3S    | ✅      | POP3 over SSL/TLS (secure)        |
| 143  | IMAP     | ❌      | Read emails (basic, unencrypted)  |
| 993  | IMAPS    | ✅      | IMAP over SSL/TLS (secure)        |

---

### 🛠️ What does the Nmap command do?

```bash
sudo nmap 10.129.14.128 -sV -p110,143,993,995 -sC
```

- `-sV`: Shows **service version** info.
- `-p...`: Scans only specified **ports** (the ones above).
- `-sC`: Uses **default scripts** for extra information (like SSL certs, service capabilities).

It checks if the mail server is running, what services are there, and whether they support secure connections (SSL/TLS).

---

### 🔍 What do the results show?

1. **Dovecot** is the mail server software.
2. Ports 110/143 are non-encrypted (POP3/IMAP).
3. Ports 995/993 are encrypted (POP3S/IMAPS).
4. The SSL certificate is **self-signed**, and shows:
   - Common name: `mail1.inlanefreight.htb`
   - Organization: `Inlanefreight`
   - Country/State: `US, California`
   - Validity: From 2021 to year 2295 😲
...
- Connects to the **IMAPS (secure IMAP)** service.
- `-k` skips SSL verification (needed because it's self-signed).
- `--user`: Login credentials.
- `-v`: Verbose mode — shows the full TLS handshake and IMAP communication.

From this, you can:
- Verify connection security (TLS v1.3 is used).
- See available **mail folders** (like `INBOX`, `Important`).
- Confirm **successful login** (`Logged in` means credentials worked).

---

### 🔓 What’s the risk if credentials are leaked?

If an attacker finds or guesses valid login credentials:
- They can **read emails**.
- They can potentially **send fake emails** (phishing).
- They may gain access to **confidential information**.

---

### 🔧 What about `openssl s_client`?
...
This lets you:
- Manually connect to the secure mail service.
- See SSL/TLS certificate details.
- Send commands manually if needed.

---

### Summary

- You're scanning email services to see what ports/protocols are open.
- You're extracting **SSL certificate info** and **service features**.
- You're testing **logins** using `curl` or `openssl` over secure connections.
- The info like CN (common name), org, and location comes from the **certificate fields**, not the Nmap tool itself.
