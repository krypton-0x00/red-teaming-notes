...
```shell
shakir0x00@htb[/htb]$ sudo nmap 10.129.2.28 -p- -sV
```
...
```shell
shakir0x00@htb[/htb]$ sudo nmap 10.129.2.28 -p- -sV --stats-every=5s
...
```shell-session
shakir0x00@htb[/htb]$ sudo nmap 10.129.2.28 -p- -sV -v 
...
```shell
shakir0x00@htb[/htb]$ sudo nmap 10.129.2.28 -p- -sV -Pn -n --disable-arp-ping --packet-trace
...
```shell
shakir0x00@htb[/htb]$ sudo tcpdump -i eth0 host 10.10.14.2 and 10.129.2.28
...
```shell 
shakir0x00@htb[/htb]$  nc -nv 10.129.2.28 25

Connection to 10.129.2.28 port 25 [tcp/*] succeeded!
220 inlane ESMTP Postfix (Ubuntu)
```

### TCP Dump - Intercept Traffic
...
```
 ...
 1. The first three lines show us the three-way handshake.
 2. After that, the target SMTP server sends us a TCP packet with the PSH and ACK flags, where PSH states that the target server is sending data to us and with ACK simultaneously informs us that all required data has been sent.
 3. The last TCP packet that we sent confirms the receipt of the data with an ACK.